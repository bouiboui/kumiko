...
class Kumiko:
    ...
    def parse_url_list(self, urls):
        ...
        for url in urls:
            filename = (
                "img" + ("0" * nbdigits + str(i))[-nbdigits:] + ".jpg"
    ...
    def parse_images(self, filenames=[], urls=None):
        ...
        for filename in sorted(filenames):
            ...
            try:
                ...
                print(
                    "Not an image, will be ignored: {}".format(filename),
                    file=sys.stderr,
                )
    ...
    def get_contours(self, gray, filename, bgcol):
        ...
        self.dbg.add_image(
            thresh, "Thresholded image, supposed {} background".format(bgcol)
        )
        ...
        for i in range(len(panels)):
            for d in ["x", "y", "r", "b"]:  # expand in all four directions
                ...
                if neighbour:
                    ...
                    newcoord = (
                        getattr(neighbour, {"x": "r", "r": "x", "y": "b", "b": "y"}[d])
                        + gutters[d]
                    )
                else:
                    # expand to the furthest known edge (frame around all panels)
                    min_panel = (
                        min(panels, key=lambda p: getattr(p, d))
                        if d in ["x", "y"]
                        else max(panels, key=lambda p: getattr(p, d))
                    )
                ...
                if newcoord != -1:
                    if (
                        d in ["r", "b"]
                        and newcoord > getattr(panels[i], d)
                        or d in ["x", "y"]
                        and newcoord < getattr(panels[i], d)
                    ):
    ...
    def parse_image(self, filename, url=None):
        ...
        infos = {"filename": url if url else os.path.basename(filename), "size": size}
        Panel.img_size = size
        ...
        if os.path.isfile(filename + ".license"):
            with open(filename + ".license") as fh:
                try:
                    ...
                    print(
                        "License file {} is not a valid JSON file".format(
                            filename + ".license"
                        )
                    )
                    sys.exit(1)
