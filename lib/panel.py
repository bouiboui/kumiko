...
class Panel:
    ...
    def __init__(self, xywh=None, polygon=None):
        ...
        for d in ["x", "y", "r", "b"]:
            super().__setattr__(
                d, 0
            )  # dummy init so that all four edges have a value (see __setattr__)
    ...
    def __eq__(self, other):
        return (
            ...
            and abs(self.y - other.y) < self.ht
            and abs(self.r - other.r) < self.wt
            and abs(self.b - other.b) < self.ht
        )
    ...
    def __str__(self):
        return "[left: {}, right: {}, top: {}, bottom: {} ({}x{})]".format(
            self.x, self.r, self.y, self.b, self.w, self.h
        )
    ...
    def __setattr__(self, name, value):
        if name in ["w", "h", "wt", "ht"]:
            raise Exception(
                "Fatal error, setting a panel's width or height is not allowed"
            )
    ...
    def is_small(self):
        return (
            self.w < Panel.img_size[0] * Panel.small_panel_ratio
            or self.h < Panel.img_size[1] * Panel.small_panel_ratio
        )
    ...
    def is_close(self, other):
        ...
        return (
            ...
            and abs(c1y - c2y) <= (self.h + other.h) * 0.75
        )
    ...
    def split(self):
        ...
        for i in range(len(self.polygon) - 1):
            ...
            for j in range(i + 1, len(self.polygon)):
                ...
                if (
                    abs(dot1[0] - dot2[0]) < max_dist
                    and abs(dot1[1] - dot2[1]) < max_dist
                ):
                    if not all_close:
        ...
        cuts = sorted(
            close_dots,
            key=lambda d: abs(self.polygon[d[0]][0][0] - self.polygon[d[1]][0][0])
            + abs(  # dot1.x - dot2.x
                self.polygon[d[0]][0][1] - self.polygon[d[1]][0][1]
            ),  # dot1.y - dot2.y
        )
