...
class Tester:
    ...
    def __init__(self, options):
        ...
        self.options = {
            ...
            if "browser" in options and options["browser"]
            else False,
    ...
    def run(self, git_version):
        ...
        tmpfolder = (
            None
            if accepts_license_files
            else tempfile.TemporaryDirectory(dir="./tests")
        )
        ...
        if git_version != "current":
            ...
            subprocess.run(
                ["git", "clone", self.git_repo, tempgit], capture_output=True
            )
            subprocess.run(
                ["git", "checkout", git_version], cwd=tempgit, capture_output=True
            )
        ...
        for f in self.files:
            ...
            print(
                "##### Kumiko-cutting {0} ({1}) #####".format(
                    f if isinstance(f, str) else f.name, git_version
                )
            )

            subprocess.run(["mkdir", "-p", os.path.join(self.savedir, git_version)])
            jsonfile = os.path.join(
                self.savedir, git_version, os.path.basename(f) + ".json"
            )
    ...
    def compare_all(self):
        ...
        for i in range(len(self.git_versions) - 1):
            ...
            print(
                "\n########## Comparing kumiko results between versions",
                v1,
                "and",
                v2,
                "##########",
            ...
            for file_or_dir in self.files:
                ...
                with open(
                    os.path.join(
                        self.savedir, v1, os.path.basename(file_or_dir) + ".json"
                    )
                ) as fh:
                    json1 = json.load(fh)
                with open(
                    os.path.join(
                        self.savedir, v2, os.path.basename(file_or_dir) + ".json"
                    )
                ) as fh:
                    json2 = json.load(fh)
